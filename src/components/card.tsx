...
export default function Card ({ file }: { file: ParchmentDocument }): JSX.Element {
  ...
  return (
    <Link href={`/d/${file._id}/edit`}>
      <a className='rounded-xl shadow-lg hover:shadow-xl focus:shadow-xl flex-1 min-w-0 mx-2 bg-white dark:bg-gray-900 box-border overflow-hidden cursor-pointer transform-gpu transition-transform duration-200 hover:scale-105 focus:scale-105 bg-clip-border border-2 border-transparent focus:border-gray-darker dark:focus:border-gray-lighter focus:outline-none' key={file._id}>
